...
def rf_cross_validation(x_train, y_train):
    ...
    cv = KFold(n_splits=5)
    clf = GridSearchCV(RandomForestClassifier(random_state=1), params, cv=cv, n_jobs=6)
...
def svm_cross_validation(x_train, y_train):
    ...
    cv = KFold(n_splits=5)
    clf = GridSearchCV(SVC(random_state=1), params, cv=cv, n_jobs=6)
    clf.fit(x_train, y_train)
    print("Best params {}".format(clf.best_params_))
    return clf.best_estimator_


def mlp_cross_validation(x_train, y_train):
    params = {
        "hidden_layer_sizes": [
    ...
    cv = KFold(n_splits=5)
    clf = GridSearchCV(MLPClassifier(random_state=1), params, cv=cv, n_jobs=6)
    clf.fit(x_train, y_train)
    print("Best params {}".format(clf.best_params_))
    return clf.best_estimator_


def log_reg_cross_validation(x_train, y_train):
    params = {
        "penalty": ['l2'],
        "tol": [1e-4, 1e-5, 1e-6],
        "C": range(1, 11, 3),
    ...
    cv = KFold(n_splits=5)
    clf = GridSearchCV(LogisticRegression(random_state=1), params, cv=cv, n_jobs=6)
    clf.fit(x_train, y_train)
    print("Best params {}".format(clf.best_params_))
    return clf.best_estimator_
...
class Run(object):
    ...
    def get_model(self, x_train, y_train):
        if self.args.grid_search and self.args.algo == 'rf':
            model = rf_cross_validation(x_train, y_train)
            ...
            model = mlp_cross_validation(x_train, y_train)
        elif self.args.grid_search and self.args.algo == 'log_reg':
            model = log_reg_cross_validation(x_train, y_train)
